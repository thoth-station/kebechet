...
# _create_issue_from_exception(manager_name=manager_name, ogr_service=ogr_service, slug=slug, exc=exc)
...
This is an automated issue generated by Kebechet. The {manager} manager threw an exception ({exception_type}) at
...
def _create_issue_from_exception(
    ...
    project = ogr_service.get_project(namespace=namespace, repo=repo)
    body = EXCEPTION_ISSUE_BODY.format(
        manager=manager_name, exception_type=exception_type, traceback=tb
    )
